...
class NvgTextInputHandler(
    ...
    fun draw(mouseX: Float, mouseY: Float, suffix: String? = null) {
        ...
        if (selectionWidth != 0f) {
            ...
            val drawX = if (sw < 0) sx + sw else sx
            NVG.rect(drawX, y + 10f, abs(sw), height - 20f, Color.cyan)
        }

        if (listening) {
            val time = System.currentTimeMillis()
            if (time - caretBlinkTime < 500) {
                NVG.rect(x + caretX + 4f - textOffset, y + height / 3.4f, 2f, 18f, Color.WHITE)
            }
    ...
    private fun textWidth(text: String): Float = NvgText.width(text)
    ...
    private fun String.removeRangeSafe(from: Int, to: Int): String = removeRange(min(from, to), max(to, from))

    private fun String.dropAt(at: Int, amount: Int): String = removeRangeSafe(at, at + amount)
}
